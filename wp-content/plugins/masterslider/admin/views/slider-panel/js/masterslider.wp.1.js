...
;(function(){
	...
	var defaults = __MSP_DEF_OPTIONS || {};
...
MSPanel.TextLayerTemplate  = '<div {{bind-attr style=view.layerStyle}}>{{{view.layer.content}}}</div>';
...
if( MSPanel.dynamicTags ){
	MSPanel.TextLayerTemplate = '<div {{bind-attr style=view.layerStyle}}>'+
...
MSPanel.ImageLayerTemplate = '{{#if view.hasImg}}<img {{bind-attr src=view.layer.img}}>{{else}}<img src="'+ __MSP_PATH +'images/image-layer.png">{{/if}}';

MSPanel.VideoLayerTemplate = '<div {{bind-attr style=view.layerStyle}} class="msp-stage-videolayer">'+
...
MSPanel.HotspotLayerTemplate = '<div class="msp-stage-hotspot"></div>';
...
MSPanel.StageLayer = Ember.View.extend({
	...
	align: function(align){
 	...
 	registerTween: function(timeline){
 		...
 		$this.css(window._jcsspfx + 'Transform' , show_eff);
 		if(this.get('layer.showFade')) $this.css('opacity' , 0);

 		// add show tween
 		var to_transform = JTween.resetTransform(show_eff);
 		var showTween = new JTween($this[0] , show_duration, {transform: to_transform , opacity: 1} , {ease: window.convertEaseName(this.get('layer.showEase')) });

 		timeline.addTween(show_delay , showTween);
 		timeline.addCallback(show_delay, this.updateOrigin, [$this, show_origin], this.updateOrigin, [$this, show_origin]);
 		...
 		var hideTween = new JTween($this[0], hide_duration, {transform:this.get('layer.hideTransform'), opacity:(this.get('layer.hideFade') ? 0 : 1)} , {ease: window.convertEaseName(this.get('layer.hideEase')) });

 		timeline.addTween(show_delay + show_duration + hide_delay, hideTween);
		timeline.addCallback(show_delay + show_duration + hide_delay, this.updateOrigin, [$this, hide_origin], this.updateOrigin, [$this, show_origin]);
	...
	updateOrigin: function(layer, origin){
		layer[0].style[window._jcsspfx + 'TransformOrigin'] = origin;
	},
...
;(function($){
	 ...
	 * @return {String}
	 */
	var getPhotostreamURL = function(user, count){
		return 'http://graph.facebook.com/' + user + '/photos/uploaded/?fields=source,name,link,images,from&limit=' + count;
	};
	 ...
	 * @return {String}
	 */
	var getAlbumURL = function(albumId , count){
		return 'http://graph.facebook.com/' + albumId + '/photos?fields=source,name,link,images,from&limit=' + count;
	};
	...
	MSPanel.FacebookController = Ember.ObjectController.extend({
		...
		loadData: function(){
			...
			if(this.get('setType') === 'album'){
				request = $.getJSON(getAlbumURL(this.get('setId') , 1) , function(data){
					that._photoData(data);
				});
			}else{
				request = $.getJSON(getPhotostreamURL(this.get('setId') , 1) , function(data){
					that._photoData(data);
				});
			}
